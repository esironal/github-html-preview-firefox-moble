...
function loadIntoWindow(aWindow) {
	// add menu item
	aWindow._gitpreviewMenuId = aWindow.NativeWindow.menu.add(getString("menu"), null, function() {
		gitpreview(aWindow);
	});
}
...
function unloadFromWindow(aWindow) {
	// remove menu item
	aWindow.NativeWindow.menu.remove(aWindow._gitpreviewMenuId);
}

function gitpreview(aWindow) {
	...
	if (url.indexOf("https://htmlpreview.github.io/?") == 0)
		return;
	...
	aWindow.BrowserApp.addTab("https://htmlpreview.github.io/?" + url, params);
}
...
function getString(aName) {
	if (!gStringBundle) {
		var uri = "chrome://gitpreview/locale/main.properties";
